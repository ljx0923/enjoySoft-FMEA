...
export default {
  ...
  methods: {
    ...
    rowDrop() {
      this.$nextTick(() => {
        ...
        this.sortable1 = Sortable.create(xTable.$el.querySelector('.body--wrapper>.vxe-table--body tbody'), {
          handle: '.drag-btn',
          // 关闭内置自动滚动，用自定义实现（内置的在 vxe-table + 原生拖拽场景下不生效）
          scroll: false,
          onStart: () => {
    ...
    // 拖拽时指针靠近表格上下边缘自动滚动
    startAutoScroll() {
      const wrapper = this.$refs.xTable1.$el.querySelector('.vxe-table--body-wrapper')
      if (!wrapper) {
        return
      }
      this._dragPointerY = 0
      const onMove = (evt) => {
        this._dragPointerY = evt.touches ? evt.touches[0].clientY : evt.clientY
      }
      this._onDragMove = onMove
      // 原生拖拽不触发 mousemove，需要同时监听 dragover
      ...
      const step = () => {
        ...
        if (y) {
          ...
          if (topDist < EDGE && wrapper.scrollTop > 0) {
            const speed = Math.max(2, Math.ceil(MAX_SPEED * (EDGE - topDist) / EDGE))
            wrapper.scrollTop = Math.max(0, wrapper.scrollTop - speed)
          } else if (bottomDist < EDGE && wrapper.scrollTop + wrapper.clientHeight < wrapper.scrollHeight) {
            const speed = Math.max(2, Math.ceil(MAX_SPEED * (EDGE - bottomDist) / EDGE))
    ...
    stopAutoScroll() {
      ...
      if (this._onDragMove) {
        ...
        document.removeEventListener('dragover', this._onDragMove)
